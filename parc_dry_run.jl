...
function run_pair_for_minit_parc(minit, paired_systems, chaotic_data;
        train_len = 10000, predict_len = 2500, n_ics = 10, rng_seed = 17, n_hp = 25)
    ...
    println("[Task $RANK] Building labelled attractors...")
...
if DRY_RUN
    ...
    minit, pair = first_job

    println("[Task $RANK] DRY RUN: running single test job with:")
    println("  minit = $minit")
    println("  pair  = $pair")

    run_pair_for_minit_parc(minit, pair, chaotic_data;
        train_len = 7000,
        predict_len = 2500,
        n_ics = 1,
        rng_seed = 1234,
